...
class DirectoryIterator(Iterator):
    ...
    #     Arguments:the files to be counted.
    #     follow_links: boolean.        
    #     Yields: tuple of (root, filename) with extension in `white_list_formats`.
    #     """
    #     def _recursive_list(subpath):return sorted(os.walk(subpath, followlinks=follow_links), key=lambda x: x[0])

    #     for root, _, files in _recursive_list(directory):
    #         for fname in sorted(files):
    #             for extension in white_list_formats:
    #                 if fname.lower().endswith('.tiff'): logging.warning('Using \'.tiff\' files with multiple bands will cause distortion. Please verify your output.')
    #     if fname.lower().endswith('.' + extension): yield root, fname

    # def _count_valid_files_in_directory(directory, white_list_formats, split,follow_links):
    #     """Count files with extension in `white_list_formats` contained in directory.
    #     Arguments:
    #     directory: absolute path to the directory containing files to be counted
    #     white_list_formats: set of strings containing allowed extensions for the files to be counted.
    #     split: tuple of floats (e.g. `(0.2, 0.6)`) to only take into
    #     account a certain fraction of files in each directory.
    #     E.g.: `segment=(0.6, 1.0)` would only account for last 40 percent
    #     of images in each directory.
    #     follow_links: boolean.
    #     Returns: the count of files with extension in `white_list_formats` contained in
    #     the directory.
    #     """
    #     num_files = len(list(_iter_valid_files(directory, white_list_formats, follow_links)))
    #     if split: start, stop = int(split[0] * num_files), int(split[1] * num_files)
    #     else:     start, stop = 0, num_files
    #     return stop - start
    ...
    def __init__(self, directory, image_data_generator,
        ...
        # print('for subdir in classes',[subdir for subdir in classes])
        ...
        for dirpath in (os.path.join(directory, subdir) for subdir in classes):
            ...
            # print('white_list_formats',white_list_formats)            
            # print('self.split',self.split)
            # print('self.class_indices',self.class_indices)
            # print('follow_links',follow_links)

        for res in results:
            classes, filenames = res.get()
            # print('res,classes,filenames',res,classes,filenames)
